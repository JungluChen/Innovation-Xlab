...
export function initChoreography(stage) {
  ...
  heads.forEach((sec) => {
    gsap.set(sec.querySelectorAll('.stage-title .char'), { yPercent: 120, autoAlpha: 0 });
    gsap.set(sec.querySelectorAll('.eyebrow, .stage-body'), { autoAlpha: 0, y: 24 });
  });

  gsap.set('.wf-card', { xPercent: -50, autoAlpha: 0, y: 90, rotationX: 7, transformPerspective: 800 });
  gsap.set('.wf-progress-num', { autoAlpha: 0, y: 10 });

  const evCards = $$('.ev-card');
  gsap.set(evCards, {
    autoAlpha: 0, scale: 1.45, y: -130,
    rotation: (i) => [-14, 10, -9][i],
    transformOrigin: '50% 0%',
  });
  gsap.set(evCards[1], { xPercent: -50, x: 0 });
  gsap.set('.method-stats', { xPercent: -50, x: 0, autoAlpha: 0, y: 44 });
  ...
  gsap.set('.relight', { xPercent: -50, yPercent: -50, autoAlpha: 0, scale: 0.85 });
  ...
  // team: students fly out of the subject's screen position
  const memberCards = $$('.member-card');
  const setTeamStart = () => {
    const shell = $('#team .stage-shell');
    const sr = shell.getBoundingClientRect();
    memberCards.forEach((card, i) => {
      const r = card.getBoundingClientRect();
      const dx = sr.left + sr.width / 2 - (r.left + r.width / 2);
      const dy = sr.top + sr.height * 0.42 - (r.top + r.height / 2);
      gsap.set(card, {
        x: dx, y: dy, scale: 0.1, autoAlpha: 0,
        rotation: [-18, 12, -9, 15, -12][i % 5],
      });
    });
  };
  setTeamStart();
  ...
  gsap.set('#act-converge .converge-line .char', { autoAlpha: 0, y: 42 });
  ...
  function headIn(tl, sel, at = 0) {
    ...
    tl.to(sec.querySelectorAll('.stage-title .char'), {
      yPercent: 0, autoAlpha: 1, duration: 0.7, stagger: 0.012, ease: 'power3.out',
    }, at + 0.05);
  ...
  function morphTo(tl, fromName, name, at = 0, dur = 1.2) {
    ...
    tl.fromTo(subject.hubPos, {
      x: (i) => F.hubs[i][0], y: (i) => F.hubs[i][1], z: (i) => F.hubs[i][2],
    }, {
      x: (i) => L.hubs[i][0], y: (i) => L.hubs[i][1], z: (i) => L.hubs[i][2],
      duration: dur, ease: 'power2.inOut', immediateRender: false,
    }, at);
    tl.fromTo(subject.satPos, {
      x: (i) => F.sats[i][0], y: (i) => F.sats[i][1], z: (i) => F.sats[i][2],
    }, {
      x: (i) => L.sats[i][0], y: (i) => L.sats[i][1], z: (i) => L.sats[i][2],
      duration: dur * 0.85, ease: 'power3.inOut', immediateRender: false,
  ...
  intro
    ...
    .fromTo(heroChars,
      ...
      {
        y: 0, rotation: 0, autoAlpha: 1, duration: 1.05,
        ease: 'back.out(1.1)', stagger: { each: 0.015, from: 'random' },
      }, 0.3)
  ...
  heroTl
    ...
    .to('#before .kicker, #before .hero-body, #before .scroll-hint', {
      autoAlpha: 0, y: -44, duration: 0.6,
    }, 0)
  ...
  convTl
    ...
    .to('#act-converge .converge-line .char', {
      autoAlpha: 1, y: 0, duration: 0.5, stagger: 0.02, ease: 'power3.out',
    }, 0.15)
    ...
    .fromTo(tubes(subject.chain), { draw: 0 }, { draw: 1, duration: 0.55, stagger: 0.18, ease: 'power2.inOut', immediateRender: false }, 1.2)
    .fromTo(subject.satScl, { v: 0 }, { v: 1, duration: 0.4, ease: 'back.out(2)', stagger: { each: 0.012, from: 'random' }, immediateRender: false }, 1.3)
    .fromTo(tubes(subject.spokes), { draw: 0 }, { draw: 1, duration: 0.5, stagger: 0.012, immediateRender: false }, 1.45)
    .to(subject.params, { labelAlpha: 1, particleAlpha: 0.95, duration: 0.4 }, 1.85)
    .to('#act-converge .converge-line .char', { autoAlpha: 0, y: -30, duration: 0.35, stagger: 0.008 }, 2.05)
  ...
  headIn(wfTl, '#workflow', 0);
  wfTl.fromTo(rain.params, { ambient: 0 }, { ambient: 0.85, duration: 1, immediateRender: false }, 0);
  for (let b = 0; b < 4; b++) {
    const at = 0.7 + b * 2.0;
    wfTl.to(cam, { x: HUBX[b] * 0.55, tx: HUBX[b] * 0.8, z: 12.2, duration: 0.9, ease: 'power2.inOut' }, at);
    wfTl.to(subject.hubActive[b], { v: 1, duration: 0.35 }, at + 0.15);
    wfTl.to(wfCards[b], { autoAlpha: 1, y: 0, rotationX: 0, duration: 0.55, ease: 'power3.out' }, at + 0.25);
  ...
  // ================= METHOD =================
  const meTl = pinTL('#methodology', 270);
  headIn(meTl, '#methodology', 0);
  morphTo(meTl, 'workflow', 'method', 0.2, 1.3);
  meTl.to(subject.params, { flowSpeed: 0.45, duration: 0.8 }, 0.2);
  drawTo(meTl, subject.threads, 0, 1, { duration: 0.7, stagger: 0.05 }, 1.0);
  const evAt = [1.5, 3.1, 4.7];
  const evRot = [-2, 1.5, -1.2];
  evCards.forEach((card, i) => {
    meTl.to(card, {
      autoAlpha: 1, scale: 1, y: 0, rotation: evRot[i],
      duration: 0.6, ease: 'back.out(1.9)',
    }, evAt[i]);
  });
  meTl.to('.method-stats', { autoAlpha: 1, y: 0, duration: 0.5 }, 6.1);
  $$('.stat-num').forEach((el) => {
  ...
  headIn(riTl, '#risk-map', 0);
  morphTo(riTl, 'method', 'risk', 0.2, 1.1);
  drawTo(riTl, subject.threads, 1, 0, { duration: 0.5, stagger: 0.02 }, 0.2);
  riTl.to(subject.params, { flowSpeed: 0.25, duration: 0.6 }, 0.3);
  riTl.to('.journey', { autoAlpha: 1, duration: 0.4 }, 0.5);
  subject.chain.forEach((c) => {
    riTl.fromTo(c.tube.mat.emissive,
      { r: SKYR.r, g: SKYR.g, b: SKYR.b },
      { r: 1, g: 0.62, b: 0.1, duration: 1.2, immediateRender: false }, 0.4);
  });
  const JCAMX = [-0.9, -0.3, 0.1, 0.7, 1.1];
  for (let b = 0; b < 5; b++) {
    const at = 1.1 + b * 2.0;
    const beat = riskBeats[b];
    if (b > 0) riTl.to(riskBeats[b - 1], { autoAlpha: 0, duration: 0.3, ease: 'power1.in' }, at - 0.2);
    riTl.to(beat, { autoAlpha: 1, duration: 0.15 }, at);
    riTl.to(beat.querySelectorAll('.risk-quote .char'), {
      autoAlpha: 1, y: 0, duration: 0.4, stagger: 0.008, ease: 'power3.out',
    }, at + 0.05);
    riTl.to(riskCards[b], { autoAlpha: 1, x: 0, duration: 0.5, ease: 'power3.out' }, at + 0.35);
    riTl.fromTo(subject.branches[b].tube, { draw: 0 }, { draw: 1, duration: 0.5, ease: 'power3.out', immediateRender: false }, at + 0.1);
    riTl.fromTo(subject.flares[b].state, { v: 0 }, { v: 1, duration: 0.4, ease: 'back.out(2.5)', immediateRender: false }, at + 0.1);
    riTl.to(subject.params, { jitter: 0.55, duration: 0.12 }, at + 0.08)
      .to(subject.params, { jitter: 0.14, duration: 0.5 }, at + 0.26);
    riTl.to(journeyDots[b], {
      backgroundColor: '#ffc000', boxShadow: '0 0 12px rgba(255,192,0,0.95)', duration: 0.2,
    }, at + 0.1);
  ...
  fxTl.to(subject.params, { jitter: 0, duration: 2 }, 1);
  ...
  for (let b = 0; b < 3; b++) {
    ...
    HEAL[b].forEach((br, k) => {
      ...
      fxTl.to(subject.pulses[br], { on: 1, duration: 0.08 }, t0)
        .fromTo(subject.pulses[br], { t: 0 }, { t: 1, duration: 0.4, ease: 'power1.in', immediateRender: false }, t0)
        .to(subject.pulses[br], { on: 0, duration: 0.15 }, t0 + 0.42);
      fxTl.fromTo(subject.branches[br].tube, { draw: 1 }, { draw: 0, duration: 0.45, ease: 'power2.in', immediateRender: false }, t0);
      fxTl.fromTo(subject.flares[br].sprite.material.color,
        { r: 1, g: 192 / 255, b: 0 },
        { r: 0.38, g: 0.74, b: 0.95, duration: 0.3, immediateRender: false }, t0);
      fxTl.fromTo(subject.flares[br].state, { v: 1 }, { v: 0, duration: 0.35, immediateRender: false }, t0 + 0.12);
    });
  }
  subject.chain.forEach((c) => {
    fxTl.fromTo(c.tube.mat.emissive,
      { r: 1, g: 0.62, b: 0.1 },
      { r: SKYR.r, g: SKYR.g, b: SKYR.b, duration: 1.4, immediateRender: false }, 3.2);
  });
  fxTl.fromTo('#vignette', { opacity: 0.5 }, { opacity: 0, duration: 0.8, immediateRender: false }, 3.4);
  ...
  imTl.fromTo(subject.params, { particleAlpha: 1, labelAlpha: 1 },
    { particleAlpha: 0, flowSpeed: 0.6, labelAlpha: 0, duration: 0.6, immediateRender: false }, 0.35);
  drawTo(imTl, subject.arcs, 0, 1, { duration: 0.5, stagger: 0.015 }, 1.3);
  imTl.to(cam, { z: 11.8, ty: 0.62, duration: 1.2, ease: 'power1.inOut' }, 0.3);
  imTl.to('.impact-left', { autoAlpha: 1, x: 0, rotationY: 0, duration: 0.6, ease: 'power3.out' }, 2.3);
  imTl.to('.impact-right', { autoAlpha: 1, x: 0, rotationY: 0, duration: 0.6, ease: 'power3.out' }, 3.5);
  imTl.to(subject.stamp, { a: 0.85, duration: 0.1 }, 4.6)
    .to(subject.stamp, { s: 1.7, duration: 0.5, ease: 'power2.out' }, 4.62)
    .to(subject.stamp, { a: 0, duration: 0.35 }, 4.85);
  imTl.to(subject.params, { glowBoost: 0.5, duration: 0.2 }, 4.6)
  ...
  teTl.fromTo(subject.satScl, { v: 1 }, { v: 0, duration: 0.5, stagger: { each: 0.02, from: 'random' }, immediateRender: false }, 1.3);
  teTl.fromTo(subject.group.position, { y: 0, z: 0 },
    { y: 1.5, z: -2.6, duration: 1.2, ease: 'power2.inOut', immediateRender: false }, 0.6);
  teTl.fromTo(subject.params, { scale: 1 }, { scale: 0.55, duration: 1.2, immediateRender: false }, 0.6);
  teTl.to('.students-label', { autoAlpha: 1, duration: 0.3 }, 1.2);
  memberCards.forEach((card, i) => {
    teTl.to(card, {
      x: 0, y: 0, scale: 1, rotation: 0, autoAlpha: 1,
      duration: 0.75, ease: 'back.out(1.25)',
    }, 1.4 + i * 0.55);
  });
  teTl.to('.mentors-block', { autoAlpha: 1, y: 0, duration: 0.6, ease: 'power1.out' }, 4.7);
  teTl.to('.mentor-card', { autoAlpha: 1, y: 0, duration: 0.6, stagger: 0.18, ease: 'power1.out' }, 4.9);
  teTl.to('.logos li', { autoAlpha: 1, y: 0, duration: 0.5, stagger: 0.12 }, 5.7);

  // ================= CTA =================
  const ctTl = pinTL('#cta', 170);
  ctTl.fromTo(subject.group.position, { y: 1.5, z: -2.6 },
    { y: 1.15, z: 0, duration: 1.2, ease: 'power2.inOut', immediateRender: false }, 0);
  ctTl.fromTo(subject.params, { scale: 0.55 },
    { scale: 0.3, glowBoost: 0.25, duration: 1.2, immediateRender: false }, 0);
  ctTl.to(subject.group.rotation, { y: 1.4, duration: 2.4, ease: 'none' }, 0);
  ctTl.fromTo(rain.params, { ambient: 0.85 }, { ambient: 0, duration: 1, immediateRender: false }, 0);
  ctTl.to(cam, { x: 0, z: 13.5, ty: 0.5, duration: 1.2, ease: 'power1.inOut' }, 0);
  ctTl.to('#cta .eyebrow', { autoAlpha: 1, y: 0, duration: 0.4 }, 0.3);
  ctTl.to('#cta .stage-title .char', {
    yPercent: 0, autoAlpha: 1, duration: 0.6, stagger: 0.012, ease: 'power3.out',
  }, 0.4);
  ctTl.to('#cta .cta-body', { autoAlpha: 1, y: 0, duration: 0.5 }, 0.8);
  ctTl.to('#cta .btn', { autoAlpha: 1, y: 0, scale: 1, duration: 0.5, stagger: 0.12, ease: 'back.out(1.7)' }, 1.05);
  ...
  const LANDING = {
    'act-converge': 0.3, workflow: 0.16, methodology: 0.16, 'risk-map': 0.13,
    findings: 0.14, impact: 0.24, team: 0.3, cta: 0.62,
  };
  function scrollToSection(id) {
    if (id === 'before') { lenis.scrollTo(0, { duration: 1.4 }); return; }
    const st = pins[id];
    const y = st
  ...
  window.addEventListener('load', () => {
    ...
    if (location.hash.length > 1) {
      ...
      requestAnimationFrame(() => {
        const st = pins[id];
        if (st) lenis.scrollTo(st.start + (st.end - st.start) * (LANDING[id] || 0.15), { immediate: true });
      });
    }
  });
